...
// Fallback: Check captions using YouTube Data API v3
async function checkCaptionsViaAPI(videoId: string): Promise<{ available: boolean; languages: any[] }> {
  console.log("Attempting to check captions via YouTube Data API...");
  ...
  try {
    ...
    if (!hasCaption) {
      console.log("API reports no captions available");
      return { available: false, languages: [] };
    }
    ...
    if (captionsResponse.ok) {
      ...
      console.log(`Found ${languages.length} caption(s) via API`);
      return { available: languages.length > 0, languages };
    }
    
    // If we can't get the list but API says captions exist, return basic info
    console.log("Captions exist but couldn't get language list");
    return { 
      available: true, 
      languages: [{ code: 'en', name: 'English', isAutoGenerated: false }] 
    };
    
  } catch (error: any) {
    console.error("Error checking captions via API:", error.message);
    return { available: false, languages: [] };
  }
}
...
async function getYouTubeTranscript(videoId: string, language: string = 'en'): Promise<string | null> {
  ...
  try {
    // Try direct caption API endpoint first (most reliable)
    const captionApiUrl = `https://www.youtube.com/api/timedtext?v=${videoId}&lang=${language}`;
    console.log(`Trying caption API: ${captionApiUrl}`);
    
    const captionApiResponse = await fetch(captionApiUrl);
    
    if (captionApiResponse.ok) {
      const xml = await captionApiResponse.text();
      ...
      if (xml && xml.trim().length > 0 && xml.includes('<text')) {
        ...
        if (texts.length > 0) {
          ...
          console.log(`Successfully extracted transcript via direct API (${transcript.length} characters)`);
          return transcript;
        }
      }
    }
    
    console.log("Direct caption API failed or returned empty, trying HTML scraping...");
...
serve(async (req: Request) => {
  ...
  try {
    ...
    if (checkOnly) {
      ...
      try {
        // First, try the direct caption API (most reliable for checking availability)
        const languages = [];
        const commonLangs = ['en', 'es', 'fr', 'de', 'ja', 'ko', 'pt', 'ru', 'it', 'zh-Hans', 'ar'];
        
        console.log("Checking captions via direct API...");
        for (const lang of commonLangs) {
          try {
            const captionApiUrl = `https://www.youtube.com/api/timedtext?v=${videoId}&lang=${lang}`;
            ...
            if (response.ok) {
              ...
              if (xml && xml.trim().length > 0 && xml.includes('<text')) {
                const langName = {
                  ...
                  'it': 'Italian', 'zh-Hans': 'Chinese (Simplified)', 'ar': 'Arabic'
                }[lang] || lang;
                
                languages.push({ code: lang, name: langName, isAutoGenerated: true });
              }
            }
          } catch (e) {
        ...
        if (languages.length > 0) {
          console.log(`Found ${languages.length} caption language(s) via direct API`);
          return new Response(
        ...
        // If direct API failed, try HTML scraping
        console.log("Direct API check complete, trying HTML scraping fallback...");
        const videoPageResponse = await fetch(`https://www.youtube.com/watch?v=${videoId}`, {
          headers: {
            'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36',
            'Accept-Language': 'en-US,en;q=0.9'
        ...
        if (videoPageResponse.ok) {
          ...
          if (captionTracksMatch) {
            const captionTracks = JSON.parse(captionTracksMatch[1]);
            const htmlLanguages = captionTracks.map((track: any) => ({
              code: track.languageCode,
              name: track.name?.simpleText || track.languageCode,
              isAutoGenerated: track.kind === 'asr'
            }));
            
            if (htmlLanguages.length > 0) {
              console.log(`Found ${htmlLanguages.length} caption(s) via HTML scraping`);
              return new Response(
                JSON.stringify({ available: true, languages: htmlLanguages }),
                { status: 200, headers: { ...corsHeaders, "Content-Type": "application/json" } }
              );
            }
          }
        ...
        // Last resort: YouTube Data API
        console.log("HTML scraping failed, trying YouTube Data API...");
        const apiResult = await checkCaptionsViaAPI(videoId);
        return new Response(
