  ...
  try {
    ...
    if (checkOnly) {
      ...
      try {
        ...
        let captionInfo: { available: boolean; languages: any[] } = { available: false, languages: [] };
        ...
        if (videoPageResponse.ok) {
          ...
          if (captionTracksMatch) {
            try {
              ...
              console.log(`Found ${languages.length} caption language(s) via HTML scraping`);
              captionInfo = { available: true, languages };
            } catch (parseError) {
        ...
        // Fallback to YouTube Data API if HTML scraping failed
        if (!captionInfo.available) {
          console.log("HTML scraping failed, trying YouTube Data API fallback...");
          captionInfo = await checkCaptionsViaAPI(videoId);
        }
        
        return new Response(
          JSON.stringify(captionInfo),
          { status: 200, headers: { ...corsHeaders, "Content-Type": "application/json" } }
        );
      } catch (error: any) {
        ...
        console.log("Exception occurred, attempting API fallback...");
